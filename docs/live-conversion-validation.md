...
| Phase 2cの計数 | 伸縮（`KeyboardAction.ResizeSegment`）は、画面に触れた1回の長押しをキー1・訂正1と数える。同じ長押しの2回目以降の繰り返しは`continued`の印を付けて数えない（`OperationClassifier`）。変換中の←→は、押した瞬間ではなく離したときに一回だけ移動する。候補の取り直しの結果は変換結果ではないため、捨てた古い結果（`stale_results_discarded`）には数えない。伸縮した文節が結果でchosenになると、`to_chosen`に数えられる。 |
