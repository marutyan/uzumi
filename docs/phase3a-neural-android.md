...
| `battery` | 電池の次の回を一つ測る。各条件を充電を切った状態で10分再生し、`dumpsys batterystats`の推定消費を読む。条件の順番はその回の方格の行、条件の間は5分空けて温度を戻す。3回で判定し、Mとの比の最小と最大が1.2をまたぐ候補があるときだけ4回目・5回目を許し、5回でもまたげば不合格とする。回どうしは別の日か、4時間以上離す（評価条件の「別の日または時間帯」を道具ではこう扱う） |
