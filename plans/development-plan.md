...
| Phase 3a 単一neural選定 | in_progress | 2026-09-24、ユーザーが「ニューラルで直接変換し、辞書で補う方式」（karukanの形）で始めると決定。llama.cpp（azooKey fork）とzenz v3.2・jinen v2の各small・xsmallを[ローカル試作](../docs/neural-probe.md)で取得・ビルドし、Macで固定8文の変換を確認（品質比較は未実施）。[設計案](../docs/neural-direct-conversion-design.md)と[評価条件の案](../docs/phase3a-model-selection-protocol.md)は承認待ち。fakeモデルでの部分範囲変換器の骨組み（かなの読みから生まれた数字・英字を辞書の候補で確かめる検査を含む）とJVMテスト10件は合格。評価条件の案は独立レビューと反証検証の指摘を反映した版2で、開発用の30文（[`phase3a-dev-tasks.tsv`](../docs/phase3a-dev-tasks.tsv)）を含む。JNI・別プロセス・端末測定は未実施 |
