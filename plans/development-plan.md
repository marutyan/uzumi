...
| Phase 2a ライブ状態 | in_progress | Android・Mozcに依存しない状態機械`dev.uzumi.ime.live`を実装し、古い結果の拒否、安定化、chosen保護、境界、ON/OFF、Undo/Redo、書記素境界を試験用変換器の決定的なイベント列32件で確認（2026-09-23）。IMEへの配線、Mozcのsegment変換との接続、実機確認は未実施 |
