...
| Phase 1c 漢字変換・辞書 | in_progress | [Mozcのnative・辞書・Java liteのローカルビルド](../docs/mozc-build-probe.md)成功。[配布物監査](../docs/mozc-distribution-audit.md)でNOTICE添付を条件に本人用debug APKへの同梱可と判定し、[`NOTICE.txt`](../third_party/mozc/NOTICE.txt)を作成。APK組込み・端末変換・ユーザー辞書は未実施 |
